...
# AUTHORITY: Approved Multi-Role Administrator Emails
PROTECTED_EMAILS = [
    ...
    'admin@stars.ae',
    '514115@naischool.ae' # Authorized Mentor/Maintainer
]
...
class STARSAPIHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            print(f">>> STARS API: GET {raw_path}")
            ...
            elif raw_path == '/api/admin/data': self.handle_admin_data_routing(); return
    ...
    def handle_dashboard_routing(self):
        try:
            ...
            if not user: self.send_error_json(401, "Not authorized"); return
    ...
    def handle_admin_data_routing(self):
        user = get_user_from_headers(self.headers)
        if not is_approved_admin(user): self.send_error_json(403, "Admin required"); return
        registry = {"users": [], "pairs": []}
        conn = sqlite3.connect(DATABASE); c = conn.cursor()
        c.execute("SELECT email, first_name, last_name, role FROM Users")
        for r in c.fetchall(): registry["users"].append({"email": r[0], "name": f"{r[1]} {r[2]}", "role": r[3]})
        ...
        for r in c.fetchall(): registry["pairs"].append({"mentor": f"{r[0]} {r[1]}", "mentee": f"{r[2]} {r[3]}", "pair_id": r[4]})
        conn.close()
        self.send_response(200); self.send_header('Content-Type','application/json'); self.end_headers()
        self.wfile.write(json.dumps(registry).encode())
    ...
    def do_POST(self):
        try:
            ...
            print(f">>> STARS API: POST {raw_path}")
            ...
            elif raw_path == '/api/delete-user' or raw_path == '/api/admin/delete': self.handle_delete_user(data)
            ...
            else: self.send_error_json(404, "Unknown Endpoint")
        except Exception as e: self.send_error_json(500, str(e))
    ...
    def do_DELETE(self):
        ...
        if self.path.startswith('/api/hard-delete'):
            try:
                table, did = params.get('table', [''])[0], params.get('id', [''])[0]
                conn = sqlite3.connect(DATABASE); c = conn.cursor()
                if table == 'users': c.execute("DELETE FROM Users WHERE email=?", (did,))
                elif 'pair' in table: c.execute("DELETE FROM MentorMenteePair WHERE id=?", (did,))
                conn.commit(); conn.close()
    ...
    def handle_login(self, data):
        ...
        if r:
            u = {"email": email, "role": r[2], "name": f"{r[0]} {r[1]}", "isCounselor": bool(r[3]), "firstName": r[0], "lastName": r[1]}
            token = str(uuid.uuid4()); SESSION_STORE[token] = u
            self.send_response(200); self.send_header('Content-Type','application/json'); self.end_headers()
            self.wfile.write(json.dumps({"success": True, "token": token, "user": u}).encode())
        else: self.send_error_json(401, "Invalid password for STARS account.")

    def handle_admin_create(self, data):
        e, f, l, r = data.get('email'), data.get('firstName'), data.get('lastName'), data.get('role')
        conn = sqlite3.connect(DATABASE); c = conn.cursor()
        c.execute("INSERT OR REPLACE INTO Users (email, first_name, last_name, role) VALUES (?, ?, ?, ?)", (e.lower(), f, l, r))
        conn.commit(); conn.close()
    ...
    def handle_delete_user(self, data):
        e = data.get('email')
        conn = sqlite3.connect(DATABASE); c = conn.cursor()
        c.execute("DELETE FROM Users WHERE email=?", (e.lower(),)); conn.commit(); conn.close()
        self.send_response(200); self.end_headers(); self.wfile.write(b'{"success": true}')

    def handle_admin_pair(self, data):
        m, s = data.get('mentor'), data.get('mentee')
        conn = sqlite3.connect(DATABASE); c = conn.cursor()
        c.execute("INSERT OR IGNORE INTO MentorMenteePair (mentor_email, mentee_email) VALUES (?, ?)", (m.lower(), s.lower()))
        conn.commit(); conn.close()
        self.send_response(200); self.end_headers(); self.wfile.write(b'{"success": true}')
    ...
    def handle_get_resources(self):
        ...
        res = [{"id": r[0], "name": r[1], "type": r[2]} for r in rows]
        self.send_response(200); self.send_header('Content-Type','application/json'); self.end_headers()
        self.wfile.write(json.dumps(res).encode())

    def get_initial_data(self):
        self.send_response(200); self.send_header('Content-Type','application/json'); self.end_headers()
        self.wfile.write(b'{"status": "Online", "v": "10.0 Final"}')
...
if __name__ == "__main__":
    init_db()
    print(f"STARS Portal v10.0 Absolute Restoration starting on port {PORT}...")
