...
async function initDashboard() {
    ...
    if (!session) return window.showAuthForm('menu');
    const { token, user } = session;
    try {
        const fname = user.firstName || user.first_name || 'STARS';
        const lname = user.lastName || user.last_name || 'User';
        const isStaff = user.role === 'ProgramStaff' || !!user.isCounselor || !!user.is_counselor;

        if (document.getElementById('profile-name')) document.getElementById('profile-name').textContent = `${fname} ${lname}`;
        if (document.getElementById('profile-initial')) document.getElementById('profile-initial').textContent = fname.charAt(0).toUpperCase();

        document.body.classList.toggle('counselor-theme', isStaff);

        const res = await fetch('/api/dashboard', { headers: { 'Authorization': `Bearer ${token}` } });
        const data = await res.json();
        window.DASH_DATA = data;

        // AUTHORED HEARTBEAT: Start session timer listener
        if (window._session_heartbeat) clearInterval(window._session_heartbeat);
        window._session_heartbeat = setInterval(() => {
            if (window.checkSessionTimers) window.checkSessionTimers();
        }, 60000);
        window.checkSessionTimers(); // Immediate initial run

        if (isStaff) {
            // CA COUNSELOR DEFAULT: Default to Mentor Responses only
            if (!window.SURVEY_FILTERS.role || window.SURVEY_FILTERS.role === 'All') {
                window.SURVEY_FILTERS.role = 'Mentor';
            }
        }
        ...
        if (sidebar) {
            const items = [
                ...
                { id: 'mentors', label: 'Find Mentors', hideForMentees: true, hideForVisitors: true },
